...
const SCALE_MATZORE = scaleFromFrets([[0,'רה'],[2,'מי'],[4,'פה#'],[5,'סול'],[7,'לה'],[9,'סי'],[11,'דו#'],[12,'רה']]);
const SCALE_SEGIAH = scaleFromFrets([[0,'רה'],[3,'רה#'],[4,'פה#'],[6,'סול#'],[7,'לה'],[9,'סי'],[10,'דו'],[12,'רה']]);
const SCALE_KARTZIGAR = scaleFromFrets([[0,'רה'],[2,'מי'],[3,'פה'],[5,'סול'],[6,'סול#'],[9,'סי'],[10,'דו'],[12,'רה']]);

function phrase(frets) {
  return frets.map(([f, s]) => dNote(f, s));
}

const MODE_LEVELS = [
  {
    id: 'm1', num: 1, name: 'רה · מי · פה · רה', subtitle: 'התחלה',
    icon: '♩', bpm: 55, unlock: 0, gameType: 'note',
    notes: phrase([[0,'רה'],[2,'מי'],[3,'פה'],[2,'מי'],[0,'רה'],[2,'מי'],[0,'רה']]),
    teach: 'שבעה צלילים על מיתר רה — בסיס לכל דרומוס. צליל אחד בכל פעם, ברור.',
